...
func TestValidateRFC1918_Valid(t *testing.T) {
	cases := []string{
		"192.168.1.11", "10.0.0.1", "172.16.0.1",
...
func TestNATCreate_InvalidPort(t *testing.T) {
	s := newNATServer(t, mockGetter{})
	result := callToolWithArgs(t, s, "freebox_nat_create", map[string]any{
		"lan_ip": "192.168.1.11", "lan_port": float64(0),
		"wan_port_start": float64(2222), "ip_proto": "tcp",
	})
	if !result.IsError {
...
func TestNATCreate_OK(t *testing.T) {
	s := newNATServer(t, mockGetter{
		"/fw/redir/": PortForwarding{ID: 5, Enabled: true, LanIP: "192.168.1.11", LanPort: 22, WanPortStart: 2222, WanPortEnd: 2222, IPProto: "tcp"},
	})
	result := callToolWithArgs(t, s, "freebox_nat_create", map[string]any{
		"lan_ip": "192.168.1.11", "lan_port": float64(22),
		"wan_port_start": float64(2222), "ip_proto": "tcp",
	})
	if result.IsError {
		t.Fatalf("tool returned error: %v", result.Content)
