...
func TestValidateDHCPIP_Valid(t *testing.T) {
	cases := []string{"192.168.100.50", "192.168.100.100", "10.0.0.10", "172.16.0.5"}
...
func TestValidateDHCPIP_ReservedRejected(t *testing.T) {
	cases := []string{
		"192.168.100.0",   // réseau
		"192.168.100.1",   // gateway
		"192.168.100.254", // Freebox
...
func TestDHCPStaticCreate_InvalidMAC(t *testing.T) {
	...
	result := callToolWithArgs(t, s, "freebox_dhcp_static_create", map[string]any{
		"mac": "not-a-mac", "ip": "192.168.100.50",
	})
	if !result.IsError {
...
func TestDHCPStaticCreate_ReservedIP(t *testing.T) {
	s := newDHCPServer(t, mockGetter{})
	result := callToolWithArgs(t, s, "freebox_dhcp_static_create", map[string]any{
		"mac": "aa:bb:cc:dd:ee:ff", "ip": "192.168.100.254",
	})
	if !result.IsError {
...
func TestDHCPStaticCreate_OK(t *testing.T) {
	s := newDHCPServer(t, mockGetter{
		"/dhcp/static_lease/": DhcpStaticLease{Mac: "aa:bb:cc:dd:ee:ff", IP: "192.168.100.50", Hostname: "mypc"},
	})
	result := callToolWithArgs(t, s, "freebox_dhcp_static_create", map[string]any{
		"mac": "aa:bb:cc:dd:ee:ff", "ip": "192.168.100.50", "hostname": "mypc",
	})
	if result.IsError {
		t.Fatalf("tool returned error: %v", result.Content)
	}
