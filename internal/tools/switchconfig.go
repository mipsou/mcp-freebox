...
func registerSwitchConfig(s *server.MCPServer, c writer) {
	...
	s.AddTool(
		...
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if err := c.Get(ctx, fmt.Sprintf("/switch/port/%d/config/", id), &cfg); err != nil {
	...
	s.AddTool(
		mcp.NewTool("freebox_switch_port_stats",
			mcp.WithDescription("Statistiques temps réel d'un port du switch : débits Rx/Tx (bytes/s), broadcasts, erreurs."),
			mcp.WithNumber("id",
				mcp.Required(),
